...
INSERT INTO `localidades` VALUES
    (1,1,'Cóbreces',601),(1,2,'Busta, La',114),(1,3,'Cigüenza',95),(1,4,'Novales',457),(1,5,'Oreña',843),
...
INSERT INTO `mancomunidades` VALUES
    ('0539002','Servicios de los Valles del Saja y Corona','Recogida y tratamiento de basuras. Asesoramiento técnico y jurídico. Servicio de extinción de incendios. Promoción turística. Promoción social y empleo. Promoción cultural. Ordenación del territorio y protección del medio ambiente',12,2),
...
INSERT INTO `concejales` VALUES
    (1,'PP',1),(2,'PP',2),(3,'PRC',3),(4,'PRC',4),(5,'PRC',5),(6,'PP',6),(7,'PP',7),(12,'PP',1),(13,'PP',1),(14,'PP',1),(15,'PP',1),
...
INSERT INTO `incluidos` VALUES
    (12,'0539002','2012-03-12'),(12,'0539016','2010-06-12'),(14,'0539002','2012-03-12'),
...
INSERT INTO `limites` VALUES
    (30,2),(4,3),(26,3),(27,3),(6,5),(29,5),(11,6),(4,10),(1,12),(23,13),(17,18),(26,19),(4,22),(26,22),(15,23),(9,24),(1,25),(4,26),
...
INSERT INTO `personajes` VALUES
    (22,'Conde de Casa Tagle de Trasierra',1,3,'Fue colonizador y destacó por su labor en la formación de pueblos indígenas'),
...
INSERT INTO `rige` VALUES
    (1,1,1,'2015-06-21',NULL,'PP'),
...
/*
1. Obtener los municipios de la comarca en los que se incluye el municipio de Ampuero
que tienen una superficie entre 30 km2 y 70 km2 y con excepción de ese municipio de Ampuero.

Municipios --> Comarca donde se incluye código --> Superficie entre 30 y 70 --> <> Ampuero
*/


SELECT *
FROM Municipios
WHERE Extension BETWEEN 30 AND 70
    AND Comarca = (
        SELECT cod_com
        FROM Comarcas
        WHERE cod_com = 'A-A'
...
/*
2. Obtener el nombre, apellidos y partido del alcalde del municipio de Cabezón de la
Sal el 2 de julio de 2009.
*/
...
SELECT partido
FROM Rige
WHERE fini < '2009-07-02' AND
    municipio = (
        SELECT id
        FROM Municipios
        WHERE nombre = 'Cabezón de la Sal ');


SELECT p.Nombre AS Nombre_Alcalde, p.Apellidos, r.partido AS Partido
FROM Personas p
INNER JOIN Alcaldes a
        ON p.idpers = a.idpers
INNER JOIN Rige r
        ON a.idpers = r.alcalde
WHERE r.Municipio = (
    SELECT m.id
    FROM Municipios m
    WHERE m.nombre = 'Cabezón de la Sal')
        AND r.fini < '2009-07-02';


/*
3. Obtener los nombres de los municipios que tienen sus actuales alcaldes registrados
junto con el nombre y apellidos de su actual alcalde y del partido al que pertenece.
*/


SELECT m.nombre AS Nombre_Municipio, p.Nombre AS Nombre_Alcalde, p.apellidos AS Apellidos_Alcalde, r.partido AS Partido
FROM Municipios m
INNER JOIN Rige r
    ON m.id = r.municipio
    AND r.ffin IS NULL
...
ON p.idpers = r.alcalde;


/*
4. Obtener los nombres de localidades de más de 200 habitantes, que tengan el texto “peña”
en su nombre, y el número de habitantes y municipio al que pertenecen.
*/


SELECT l.nombre_loc AS 'Nombre Localidad', l.habitantes AS 'Nº Habitantes', m.nombre AS 'Nombre Municipio'
FROM Localidades l
INNER JOIN Municipios m
    ON l.municipio = m.id
WHERE l.habitantes > 200 AND l.nombre_loc LIKE '%PEÑA';


/*
5. Obtener las asociaciones a las que pertenece el municipio de Lamasón y cuál es la
capital de estas asociaciones.
*/

-- Mancomunidades (asociaciones) --> Municipios --> Incluidos (es lo que nos da la relación entre municipios
-- y asociaciones, de forma que si el id de un municipio está ahí, está registrado)


SELECT ma.nombre_manc AS Nombre_Asociación, muni1.nombre AS Capital
FROM Mancomunidades ma
INNER JOIN Municipios muni1
    ON ma.municipio_sede = muni1.id
WHERE ma.reg_manc = (
    SELECT i.mancomunidad
    FROM Incluidos i
    WHERE i.municipio = (
        SELECT muni2.id
        FROM Municipios muni2
        WHERE muni2.nombre = 'Lamasón'
        )
    );


/*
6. Obtener, para cada municipio de las regiones de código COC, LIE o SN, cuál es la localidad
con mayor población y cuál es su población.

Tenemos que relacionar 2 tablas: Municipios, Localidades y a la vez sacar el máximo de habitantes
por municipio.

La relación entre tablas la logramos con INNER JOINS. La primera es para vincular los ID's de municipios
de Municipios y Localidades. Luego la segunda es necesaria para que podamos sacar también el nombre de la
localidad y el máximo de habitantes. Es como guardar una subquery en el seudónimo AS
*/


SELECT municipio, max(habitantes)
FROM Localidades
GROUP BY municipio; -- Sacamos una "relación" artifical a la que hacerle un INNER JOIN


SELECT m.nombre AS Nombre_Municipio, l.nombre_loc AS Nombre_Localidad, l.habitantes AS Habitantes
FROM Municipios m
INNER JOIN Localidades l
    ON m.id = l.municipio
INNER JOIN (
    SELECT municipio, max(habitantes) AS Max_Habitantes
    FROM Localidades l2
    GROUP BY municipio
) AS Max_Habit_por_Municipio
    ON l.municipio = Max_Habit_por_Municipio.municipio
        AND l.habitantes = Max_Habit_por_Municipio.Max_Habitantes
WHERE m.comarca IN ('COC', 'LIE', 'S-N')
...
SELECT m.nombre AS Municipios_Sin_Localidad
FROM Municipios m
WHERE m.id NOT IN (
    SELECT l.municipio
    FROM Localidades l
...
Para poder sacar la población de cada municipio a partir de todas sus localidades, no solo tenemos que hacer un
GROUP BY por municipios, sino también sumar todos los habitantes. De esta forma podremos realizar el GROUP BY. Ya que
de lo contrario nos devuelve más de 1 tupla
*/


SELECT m.nombre AS Nombre_Municipio, sum(l.habitantes) AS Poblacion_Total
FROM Municipios m
INNER JOIN Localidades l
    ON m.id = l.municipio
GROUP BY m.nombre
...
-- Aunque sólo haya 27 tuplas, gracias a la Query de antes podemos ver que hay municipios sin localidades registradas.
-- Para ser precisos, 75. Y 27 + 75 = 102; que es el máximo de tuplas en Municipios. Así que la Query *debería* estar bien


/*
9. Crear una VISTA denominada "poblacion" que obtenga la población de cada municipio a partir
de la población de sus localidades. La vista debe tener 3 columnas: Identificación del
municipio, nombre del municipio y la suma de los habitantes de las localidades de ese
municipio.

Mostrar todos los resultados de la VISTA con SELECT * FROM poblacion;
(Sugerencia: usar un alias para SUM (habitantes)).
*/


CREATE VIEW Poblacion AS
SELECT m.id AS ID_Municipio, m.nombre AS Nombre_Municipio, sum(l.habitantes) AS Poblacion_Total
FROM Municipios m
INNER JOIN Localidades l
    ON m.id = l.municipio
...
/*
10. Obtenga cuál de los municipios tiene más población. Debe hacerse desde los habitantes
de sus localidades. (Consejo: usar la vista anterior para calcular la población primero y
luego usar esa vista para calcular el MAX).
*/
...
WHERE Poblacion_Total = (
    SELECT max(Poblacion_Total)
    FROM Poblacion
    );


/*
11. Obtener la localidad menos poblada del municipio más poblado.
(Sugerencia: usar la vista anterior)
*/


SELECT l.municipio AS Municipio, l.nombre_loc AS Localidad, l.habitantes AS Habitantes
FROM Localidades l
INNER JOIN Poblacion p
    ON l.municipio = p.ID_Municipio
WHERE p.Poblacion_Total = (
    SELECT max(Poblacion_Total)
    FROM Poblacion
    )
ORDER BY Habitantes
...
-- Las vistas lo simplifican un montón \o/ que maravilla

/*
12. Obtener los municipios con mayor densidad poblacional (población / extensión) que
el municipio de Ruente. (Sugerencia: usar la vista anterior)
*/
...
SELECT m.nombre AS Nombre_Municipio, p.Poblacion_Total AS Poblacion
FROM Poblacion p
INNER JOIN Municipios m
    ON p.ID_Municipio = m.id
WHERE p.Poblacion_Total / m.extension > (
    SELECT p2.Poblacion_Total / m.extension
    FROM Poblacion p2
...
/*
SELECT m.nombre AS Nombre_municipio, count(l.nombre_loc) AS Cantidad_Localidades
FROM Municipios m
INNER JOIN Localidades l
    ON m.id = l.municipio
GROUP BY m.nombre
HAVING Cantidad_Localidades < 10;

Había leído cantidad de localidades en vez de habitantes :) Así que bueno ya dejo esto aquí
...
SELECT count(partido) AS Cantidad_Concejales, m.nombre AS Nombre_Municipio, c.partido AS Partido
FROM Concejales c
INNER JOIN Municipios m
    ON c.municipio = m.id
GROUP BY m.nombre, c.partido
ORDER BY Cantidad_Concejales DESC ;


/*
15. Obtener los nombres y extensión de los municipios que colindan con el municipio de
Cillorigo de Liébana.
(Consejo: usar UNION)

Colindan = cercanos. Límite geográfico lo da la relación Limites

Si el municipio 20 y 33 colindan, estarán en esta tabla.
*/


SELECT m1.nombre AS Nombre_Municipio, m1.extension AS Extensión
FROM Municipios m1
INNER JOIN Limites l1
        ON m1.id = l1.municmenor
WHERE l1.municmayor = (
    SELECT m2.id
    FROM Municipios m2
...
SELECT m3.nombre AS Nombre_Municipio, m3.extension AS Extensión
FROM Municipios m3
INNER JOIN Limites l2
        ON m3.id = l2.municmayor
WHERE l2.municmenor = (
    SELECT m4.id
    FROM Municipios m4
    WHERE m4.nombre = 'Cillorigo de Liébana'
    );

-- El UNION en este caso hace falta por cómo está construída la relación Límites. Debido a que
-- tenemos que seleccionar todos los municipios que colindan, ya sean menores o mayores
-- en límites, tenemos que hacer un SELECT de los menores y luego de los mayores y unirlos.